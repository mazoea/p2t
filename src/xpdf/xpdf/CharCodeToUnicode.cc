...
int CharCodeToUnicode::mapToUnicode(CharCode c, Unicode *u, int size) {
  ...
  if (!map) {
    u[0] = (Unicode)c;
    globalParams->changeIfExtendedSpace(u, 1);
    return 1;
  }
  if (c >= mapLen) {
	// put the original into u for later use - accented
    u[0] = u[1] = accented::word::UNICODE_UNKNOWN_PREFIX;
    u[2] = c;
    return 0;
  }
  if (map[c]) {
  ...
  for (i = 0; i < sMapLen; ++i) {
	...
	u[j] = sMap[i].u[j];
      ...
      globalParams->changeIfExtendedSpace(u, j);
      return j;
    }
  }
  // put the original into u for later use - accented
  if (2 < size) {
    // is it a normal ascii letter?
    // - no unicode mapping but still valid ascii
    if ( (char('\n')  != c 
            && char('\t')  != c
            && char('\b')  != c
            && char('\r')  != c
