...
void GlobalParams::parseLine(char *buf, GString *fileName, int line) {
  ...
  if (tokens->getLength() > 0 &&
	...
	if ((f2 = openFile(incFile->getCString(), "rb"))) {
...
void
GlobalParams::setExtendedSpace( int* it_start, size_t len )
{
  white_spaces = new Unicode[len + 1]();
  size_t i = 0;
  while( i < len )
  {
    white_spaces[i] = (Unicode)(*it_start);
    ++it_start;
    ++i;
  }
}
...
void
GlobalParams::changeIfExtendedSpace( Unicode* code, size_t len ) const
{
  if ( white_spaces && len == 1)
  {
    for (auto it = white_spaces; *it != 0; ++it)
    {
      if (code[0] == *it) {
        code[0] = ' ';
        break;
