...
#include "xpdf/UnicodeMapAccent.h"
...
// Maximum position difference in content stream for words to coalesce
// - it is again a hack for xpdf to be more pdf friendly, not a real fix because that
// is clearly impossible in this code.
#define maxSameWordPosDiff 4
...
namespace {
    ...
    int dumpFragment(Unicode *text, int len, UnicodeMap *uMap, GString *s, GBool primaryLR)
    {
    ...
    std::string str(Unicode* text, int len, GBool primaryLR)
    {
        std::string ret;
        {
            auto tmpMap = globalParams->getTextEncoding();
            std::shared_ptr<GString> ps(new GString);
            dumpFragment(text, len, tmpMap, ps.get(), true);
            tmpMap->decRefCnt();
            ret = ps->getCString();
        }
        return ret;
...
TextWord::TextWord(GfxState *state, int rotA, double x0, double y0,
		   TextFontInfo *fontA, double fontSizeA) : xMax(-1.0), yMax(-1.0), xMin(-1.0), yMin(-1.0)
{
  GfxFont *gfxFont;
...
void TextLine::coalesce(UnicodeMap *uMap) {
  ...
  if (debug) dbg_ = str(this->text, this->len, true);
}
...
void TextBlock::coalesce(UnicodeMap *uMap, double fixedPitch) {
  ...
  while (1) {
	...
	for (word0 = NULL, word1 = pool->getPool(baseIdx);
      ...
      // 40ptx + over - give it more room
      double factor = (sqrt(fontSize/10) - 2) > 0 ? sqrt(fontSize/10) : 1;
      double min_spacing = minCharSpacing * factor;
	    if (delta < min_spacing * fontSize) {
	      overlap = gTrue;
	      break;
	    } else {
...
void TextPage::addChar(GfxState *state, double x, double y,
  ...
  // if w=h=0 - continue
  if ( 0 == w1 && 0 == h1 ) {
	error(errSyntaxError, 0, "width==height==0 so we are not using that char [%u]", u[0] );
    charPos += nBytes;
	return;
  }

  // 
  GBool tmp_char_overlaps = gFalse;


  // accented wrapper
  accented::word accented_word( curWord, u, x1, y1, w1, h1, curFontSize );
  ...
  // (4) the font size has changed
	bool should_add_char = true;

  if (curWord && curWord->len > 0) {


  // space_indicating_word_end is also in negative (special overlapping)
  if ( accented_word.can_translate_accent() ) {
		// set the last char and do not add the char in the normal way
		// - we do not know charcode (it is not present otherwise it would be used)
		// - also take into account that we want the bbox of the letter not the accent
        // A`
        int found_translation = false;
        Unicode translated_char = accented_word.translate_to_one_unicode(found_translation);
        if ( UnicodeMapAccent::FOUND == found_translation ) 
        {
		    if ( accented_word.last_is_accent() ) {
			    --curWord->len;
			    (const_cast<Unicode*>(u))[0] = translated_char;
                accented::word::last_was_translated( true );
            // `A
		    }else {
			    curWord->text[curWord->len-1] = translated_char;
			    should_add_char = false;
		    }
        
        // do nothing
        }else if ( UnicodeMapAccent::DO_NOT_TRANSLATE == found_translation ) 
        {
				assert( true && "debugging" );
        
        // if no real conversion was made put there at least the letter
        }else {
			    --curWord->len;
			    (const_cast<Unicode*>(u))[0] = translated_char;
                accented::word::last_was_translated( true );
	...
	// if there is a new word starting with accent, it is on the same line and not too far away
	// we also use space_indicationg_word_end; otherwise, it would not fall into endWord
	}else if ( accented_word.new_word_starts_with_accent( u[0] ) ) {

	// if there was an accented and the next one overlaps
	// leave it as one word
	}else if ( accented_word.last_translated_new_overlaps() ) {

	// normal chars
    }else {

        if ( accented_word.should_end_word() || lastCharOverlap ) 
		{
		    accented_word.endingWord( curWord );
			endWord();
		}
		tmp_char_overlaps = accented_word.overlap();
	}
  }
  lastCharOverlap = tmp_char_overlaps;

    // this is another hack
    // - if we could not find a unicode char (see mapToUnicode) we
    // hacked u and put -1 to u[0] and u[1] and the code to u[2]
    // - must be here because if a char starts a new word we end up here
    //
    if ( accented_word.is_invalid_unicode( u ) ) {
        Unicode mapped_char = accented_word.translate_invalid_unicode( u, state );
        uLen = 1;
        ((Unicode *)(u))[0] = mapped_char;
        ((Unicode *)(u))[1] = ((Unicode *)(u))[2] = 0;
    }


  if (uLen != 0 && should_add_char) {
    // start a new word if needed
    if (!curWord) {
      beginWord(state, x, y);
	  accented_word.beganWord( curWord );
    }
...
void TextPage::coalesce(GBool physLayout, double fixedPitch, GBool doHTML) {
  ...
  if (debug)
  {
      ...
      {
          auto& b = blocks[i];
          TextLine* tls = b->getLines();
          while (tls)
          {
              dbg_.push_back(str(tls->text, tls->len, true));
              tls = tls->next;
          }
      }
  }
...
GString *TextPage::getText(double xMin, double yMin,
  ...
  if (nFrags > 0) {
	...
	s->append(space, spaceLen);
      ...
      col += dumpFragment(frag->line->text + frag->start, frag->len, uMap, s, primaryLR);
    }
...
void TextPage::dump(void *outputStream, TextOutputFunc outputFunc,
		    GBool physLayout, frag_listener* listener) {
  UnicodeMap *uMap;
  TextFlow *flow;
  TextBlock *blk;
  TextLine *line;
  ...
  if (rawOrder) {
    ...
    for (word = rawWords; word; word = word->next) {
      s = new GString();
      dumpFragment(word->text, word->len, uMap, s, primaryLR);
      (*outputFunc)(outputStream, s->getCString(), s->getLength());
      // jm
      if (listener)
        listener->line(word, s->getCString(), s->getLength());
      delete s;
      if (word->next &&
    ...
    // jm
    if (listener)
      listener->space( spaceLen );
	}
      } else {
	(*outputFunc)(outputStream, eol, eolLen);
    // jm
    if (listener)
      listener->end_line( eolLen );
      }
    }
  ...
  // output the page, maintaining the original physical layout
	...
	(*outputFunc)(outputStream, space, spaceLen);
      ...
      // jm
      if (listener)
        listener->line(frag, s->getCString(), s->getLength());
  ...
  // output the page, "undoing" the layout
	...
	for (line = blk->lines; line; line = line->next) {
	  ...
	  s = new GString();
	  dumpFragment(line->text, n, uMap, s, primaryLR);
	  (*outputFunc)(outputStream, s->getCString(), s->getLength());
      // jm
      if (listener)
        listener->line(line, s->getCString(), s->getLength());
...
TextOutputDev::TextOutputDev(TextOutputFunc func, void *stream,
			     ...
			     GBool rawOrderA) : listener_(NULL) {
  UnicodeMapAccent::initialise();
  outputFunc = func;
  outputStream = stream;
  needClose = gFalse;
...
void TextOutputDev::startPage(int pageNum, GfxState *state) {
  ...
  if (this->listener_)
    this->listener_->start_page( pageNum, state );
}
...
void TextOutputDev::endPage() {
  ...
  if (outputStream) {
    text->dump(outputStream, outputFunc, physLayout, this->listener_);
  }
}
                          ...
                          int *maskColors, GBool inlineImg, GBool interpolate)
{
    if (this->listener_)
    {
        this->listener_->image(
            static_cast<int>(state->getCurX()), 
            static_cast<int>(state->getCurY()), 
            width, height, (gTrue == inlineImg));
    }
    OutputDev::drawImage(state, ref, str, width, height, colorMap, maskColors, inlineImg, interpolate);
}